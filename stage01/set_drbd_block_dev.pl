...
if ($ws_count < 2) {
    print "Need two walruses, skipping stage\n";
    exit(0);
}
...
print "$clc_ip :: source /root/eucarc; /usr/sbin/euca-modify-property -p walrus.blockdevice=/dev/drbd1\n";
system("ssh -o StrictHostKeyChecking=no root\@$clc_ip \"source /root/eucarc; /usr/sbin/euca-modify-property -p walrus.blockdevice=/dev/drbd1\" ");
sleep(1);
...
print "$clc_ip :: source /root/eucarc; /usr/sbin/euca-modify-property -p walrus.resource=r0\n";
system("ssh -o StrictHostKeyChecking=no root\@$clc_ip \"source /root/eucarc; /usr/sbin/euca-modify-property -p walrus.resource=r0\" ");
sleep(1);
